    ...
    <div className="container mx-auto px-4 py-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {products.map((product, index) => (
          <div
            key={index}
            className="bg-white border rounded-lg overflow-hidden shadow-md transition-transform duration-300 transform hover:shadow-green-300 hover:scale-105"
          >
            <img src={product.img} alt={product.name} className="w-full h-48 object-cover" />
            <div className="p-4">
              <h3 className="text-md text-gray-500 text-center">{product.name}</h3>
              <p className="text-sm text-gray-300  text-center">12 Pc(s)</p>
              ...
              <div className="flex items-center">
                <button className='flex-1 bg-white text-green-600 text-lg font-bold'>
                  Details
                </button>
              </div>
              <div className="flex items-center mt-2">
                <button className="flex-1 text-white bg-green-500 py-1 px-4 rounded-lg hover:bg-green-600 hover:shadow-md transition duration-300">
                  <FontAwesomeIcon icon={faCartPlus} className="mr-2" /> Add to Cart
                </button>
              </div>
              <div className="flex bg-green-500 items-center mt-2 justify-between rounded-lg">
                <button className="text-white bg-green-600 py-1 px-3 rounded-lg hover:bg-green-600 transition duration-300">
