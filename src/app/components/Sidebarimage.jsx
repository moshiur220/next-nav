...
const RecursiveMenu = ({ menu, openMenus, toggleMenu, level = 0 }) => (
  <ul className={`ml-${level * 2} space-y-2 transition-all duration-300 ease-in-out`}>
    {menu.map((item, index) => (
      <li key={index}>
        <div
          className={`flex items-center justify-between cursor-pointer p-2 rounded ${
            openMenus.includes(item.name) ? 'bg-violet-600 text-white' : 'text-black hover:bg-indigo-600 hover:text-white'
          }`}
          onClick={() => {
        ...
        {openMenus.includes(item.name) && item.subMenu && (
          <RecursiveMenu menu={item.subMenu} openMenus={openMenus} toggleMenu={toggleMenu} level={level + 1} />
        )}
      </li>
    ))}
  </ul>
  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-4">
          <RecursiveMenu menu={menuData} openMenus={openMenus} toggleMenu={toggleMenu} />
        </div>
      </div>
